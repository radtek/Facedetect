...
import com.example.hzmt.facedetect.util.AccessControlUtil;

//import com.example.hzmt.idcardfdv.IdcardFdv;
import com.example.hzmt.facedetect.util.IdcardFdv;
...
public class FaceTask extends AsyncTask<Void, Void, FaceDetector.Face>{
    ...
    private static class IdcardReadThead extends Thread {
        ...
        public void run() {
            ...
            // Log.e("BluetoothDevice", "bonded device size ="+devices.size());
            String devname = new String("INVS300-05090189");
            String devaddr = null;
            for(BluetoothDevice bonddevice:devices){
                if(bonddevice.getName().equals(devname)){
                    devaddr = bonddevice.getAddress();
                    break;
                }
            }
            //Log.e("INVS300:", "开始连接");
            if(null == MyApplication.MyBtReaderClient) {
                MyApplication.MyBtReaderClient = new BtReaderClient(context);
                IClientCallBack iccb = new IClientCallBack() {
                    @Override
                    public void onBtState(boolean is_connect) {
                        if (is_connect) {
                            //Log.e("INVS300:", "保持连接");
                            MyApplication.isBtReaderClientConnected = true;
                        } else {
                            //Log.e("INVS300:", "未连接");
                            MyApplication.isBtReaderClientConnected = false;
                        }

                    }
                };
                MyApplication.MyBtReaderClient.setCallBack(iccb);
            }
            BtReaderClient mClient = MyApplication.MyBtReaderClient;
            //if(MyApplication.isBtReaderClientConnected == false) {
                if (!mClient.connectBt(devaddr)) {
                    Log.e("INVS300:", "连接失败");
            ...
            //}
                ...
                if(card != null){
                    Log.e("INVS300:", card.getName());
                    CameraActivityData.Idcard_id = card.getIdNo();
                    CameraActivityData.Idcard_issuedate = card.getStart();
                    ...
                    if ((szBmp != null) && (szBmp.length == 38862)){
                        ...
                        //intent.setClass(mActivity, SubActivity.class);
